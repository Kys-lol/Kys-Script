...
if not ModernV2 then
    ...
    if loaderOk then
        ...
        warn("[MawwwHub] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        ...
        if fallbackOk then
            ...
            warn("[MawwwHub] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "MawwwHub_UI_Theme.json"
local MawwwUI_Solid = false
local MawwwUI_Color = "Blue"
...
pcall(function()
    if isfile and readfile and isfile(UIConfigPath) then
        ...
        MawwwUI_Color = data.Color or "Blue"
    end
end)
...
if ModernV2 then
    ...
    Window = ModernV2:Window({
        ...
        Config = {
            ConfigFolder = "MawwwHubViolenceDistrict",
            AutoSaveFile = "MAWWW_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
            Format = "JSON",
    ...
    Window:SetAccount({
        ...
        Expires = "MawwwHubOnTop",
    })

    Window:CreateHomeTab({
        Name = "Dashboard",
        Icon = "lucide:layout-dashboard",
        Content = "MawwwHub Violence District Script",
...
getgenv().VD = getgenv().VD or {
    ...
    SPEAR_Gravity         = 50,
    SPEAR_Speed           = 100,
    -- Silent Veil Spear (migrated from Gluto)
...
if VD.FovRadius == nil then VD.FovRadius = 150 end
if VD.MaxTargetDistance == nil then VD.MaxTargetDistance = 500 end
if VD.CustomSpearSpeed == nil then VD.CustomSpearSpeed = 165 end
if VD.CustomSpearGravity == nil then VD.CustomSpearGravity = 103 end
if VD.LeadMultiplier == nil then VD.LeadMultiplier = 1.4 end
if VD.AuraSpearSpeed == nil then VD.AuraSpearSpeed = 165 end
if VD.AuraSpearGravity == nil then VD.AuraSpearGravity = 96.5 end
...
local ConfigFolderName = "MawwwHub_VD"
local HttpService = game:GetService("HttpService")
...
local VD_To_Flag = {
    ...
    EnableSilentVeil = "Veil Enable Silent Veil",
    AutoPredict = "Veil Auto Predict",
    FovRadius = "Veil FOV Radius (pixels)",
    MaxTargetDistance = "Veil Max Target Distance (studs)",
    ShowTracker = "Veil ESP Tracker Target",
    ShowFOV = "Veil Show FOV Circle",
    CustomSpearSpeed = "Veil Spear Speed (Normal)",
    CustomSpearGravity = "Veil Spear Gravity (Normal)",
    LeadMultiplier = "Veil Lead Multiplier",
    AuraSpearSpeed = "Veil Aura Spear Speed",
    AuraSpearGravity = "Veil Aura Spear Gravity",
    ...
    VIS_MawwwtKiller = "Mawwwt Killer Display",
...
do
    ...
    getgenv().MAWWW_AddVisualESPControls = function(VisualTabRef)
        ...
        playerSection:AddToggle({
            ...
            Flag = "MAWWW Player Distance ESP",
            Default = false,
            Callback = function(state)
...
local MAWWW_HideSurvivorIconImage = "rbxassetid://95802749943553"
local MAWWW_HideSurvivorIconText = "NxH"
...
-- ============================================================
-- SILENT VEIL SPEAR AIMBOT (L2-HUB STYLE) - UPDATED WITH GREEN + OUTLINE
-- ============================================================
local SpearState = {
    target = nil,
    lookVector = nil,
...
-- Visuals dengan outline hitam dan fill hijau
local SpearVisuals = {}
pcall(function()
    -- FOV Circle: outline hitam (lebih tebal) + fill hijau
    SpearVisuals.FOVCircleOutline = Drawing.new("Circle")
    SpearVisuals.FOVCircleOutline.Color = Color3.fromRGB(0, 0, 0)
    SpearVisuals.FOVCircleOutline.Thickness = 3
    SpearVisuals.FOVCircleOutline.Filled = false
    SpearVisuals.FOVCircleOutline.Transparency = 0.3
    SpearVisuals.FOVCircleOutline.Visible = false

    SpearVisuals.FOVCircleFill = Drawing.new("Circle")
    SpearVisuals.FOVCircleFill.Color = Color3.fromRGB(0, 255, 0)
    SpearVisuals.FOVCircleFill.Thickness = 1
    SpearVisuals.FOVCircleFill.Filled = false
    SpearVisuals.FOVCircleFill.Transparency = 0.5
    SpearVisuals.FOVCircleFill.Visible = false

    -- Tracker Circle: outline hitam + fill hijau
    SpearVisuals.TrackerCircleOutline = Drawing.new("Circle")
    SpearVisuals.TrackerCircleOutline.Color = Color3.fromRGB(0, 0, 0)
    SpearVisuals.TrackerCircleOutline.Thickness = 3
    SpearVisuals.TrackerCircleOutline.Filled = false
    SpearVisuals.TrackerCircleOutline.Transparency = 0.3
    SpearVisuals.TrackerCircleOutline.Visible = false

    SpearVisuals.TrackerCircleFill = Drawing.new("Circle")
    SpearVisuals.TrackerCircleFill.Color = Color3.fromRGB(0, 255, 0)
    SpearVisuals.TrackerCircleFill.Thickness = 1
    SpearVisuals.TrackerCircleFill.Filled = false
    SpearVisuals.TrackerCircleFill.Transparency = 0.5
    SpearVisuals.TrackerCircleFill.Visible = false

    -- Tracker Line (hijau)
    SpearVisuals.TrackerLine = Drawing.new("Line")
    SpearVisuals.TrackerLine.Color = Color3.fromRGB(0, 255, 0)
    SpearVisuals.TrackerLine.Thickness = 2
    SpearVisuals.TrackerLine.Transparency = 0.5
    SpearVisuals.TrackerLine.Visible = false
end)
...
Players.PlayerRemoving:Connect(function(p)
    if p.Character then SpearState.velHistory[p.Character] = nil end
end)
...
local function UpdateSpearAimbot()
    if GetRole() ~= "Killer" then
        ...
        if SpearVisuals.FOVCircleFill then SpearVisuals.FOVCircleFill.Visible = false end
        if SpearVisuals.FOVCircleOutline then SpearVisuals.FOVCircleOutline.Visible = false end
        if SpearVisuals.TrackerCircleFill then SpearVisuals.TrackerCircleFill.Visible = false end
        if SpearVisuals.TrackerCircleOutline then SpearVisuals.TrackerCircleOutline.Visible = false end
    ...
    if VD.ShowFOV and VD.EnableSilentVeil then
        SpearVisuals.FOVCircleOutline.Position = center
        SpearVisuals.FOVCircleOutline.Radius = VD.FovRadius or 150
        SpearVisuals.FOVCircleOutline.Visible = true
        SpearVisuals.FOVCircleFill.Position = center
        SpearVisuals.FOVCircleFill.Radius = VD.FovRadius or 150
        SpearVisuals.FOVCircleFill.Visible = true
    else
        SpearVisuals.FOVCircleOutline.Visible = false
        SpearVisuals.FOVCircleFill.Visible = false
    end

    if not VD.EnableSilentVeil then
        SpearState.target = nil
        SpearState.lookVector = nil
        SpearVisuals.TrackerCircleOutline.Visible = false
        SpearVisuals.TrackerCircleFill.Visible = false
    ...
    local nearest = nil
    local bestDist = VD.FovRadius or 150
    local bestStudDist = VD.MaxTargetDistance or 500
    ...
    if nearest and nearest.Character then
        ...
        if tpPart then
            ...
            if dist > 0.1 and dist <= (VD.MaxTargetDistance or 500) then
                local isAuraActive = char:GetAttribute("special") == true
                local prof

                if isAuraActive then
                    prof = {
                        v0 = VD.AuraSpearSpeed or 165,
                        g = VD.AuraSpearGravity or 96.5,
                        windup = 0.10,
                        latency = 0.04,
                        maxlead = 25,
                        scale = VD.LeadMultiplier or 1.4
                    }
                else
                    prof = {
                        v0 = VD.CustomSpearSpeed or 165,
                        g = VD.CustomSpearGravity or 103,
                        windup = 0.10,
                        latency = 0.04,
                        maxlead = 45,
                        scale = VD.LeadMultiplier or 1.4
                ...
                local aimPoint = tp
                if VD.AutoPredict then
                    local vel = getCharacterVelocity(nearest.Character)
                ...
                if ahDist > 0.001 then
                    SpearState.lookVector = ah.Unit * math.cos(pitch) + Vector3.new(0, math.sin(pitch), 0)
                else
                    SpearState.lookVector = adir.Unit
                end
                SpearState.target = nearest

                if VD.ShowTracker then
                    local sp, vis = cam:WorldToScreenPoint(tpPart.Position)
                    if vis and sp.Z > 0 then
                        local screenDist = (Vector2.new(sp.X, sp.Y) - center).Magnitude
                        if screenDist <= (VD.FovRadius or 150) then
                            local bottomCenter = Vector2.new(center.X, cam.ViewportSize.Y)
                            -- Line
                            SpearVisuals.TrackerLine.From = bottomCenter
                            SpearVisuals.TrackerLine.To = Vector2.new(sp.X, sp.Y)
                            SpearVisuals.TrackerLine.Visible = true
                            -- Circle outline + fill
                            local radius = math.clamp(1000 / dist, 10, 40)
                            SpearVisuals.TrackerCircleOutline.Position = Vector2.new(sp.X, sp.Y)
                            SpearVisuals.TrackerCircleOutline.Radius = radius
                            SpearVisuals.TrackerCircleOutline.Visible = true
                            SpearVisuals.TrackerCircleFill.Position = Vector2.new(sp.X, sp.Y)
                            SpearVisuals.TrackerCircleFill.Radius = radius
                            SpearVisuals.TrackerCircleFill.Visible = true
                        else
                            SpearVisuals.TrackerLine.Visible = false
                            SpearVisuals.TrackerCircleOutline.Visible = false
                            SpearVisuals.TrackerCircleFill.Visible = false
        ...
        pcall(function()
            ...
            oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if not checkcaller() then
                    ...
                    if method == "GetAttributes" and (VD.KILLER_InfLakeMist or VD.KILLER_InfPursuit) then
                    ...
                    if method == "FireServer" then
                        if self.Name == "Spearthrow" and VD.EnableSilentVeil and typeof(SpearState.lookVector) == "Vector3" and GetRole(LocalPlayer) == "Killer" then
                            if typeof(args[1]) == "Vector3" then args[1] = SpearState.lookVector end
                            return oldNamecall(self, unpack(args))
                        end
                        if VD.KILLER_InfLakeMist and self.Name == "LakeMist" then
                            local a1=args[1]
                            if a1==false then return nil elseif a1==true then
                                task.delay(0.2,function() pcall(function() local c=game:GetService("Players").LocalPlayer.Character; if c and c:GetAttribute("action")==true then c:SetAttribute("action",false) end end) end)
                            end
                        end
...
do -- Aim Tab
    ...
    spearSection:AddToggle({ Default = VD.EnableSilentVeil or false, Name = "Enable Silent Veil", Flag = "Veil Enable Silent Veil", Callback = function(v)
        VD.EnableSilentVeil = v
        pcall(VD_Notify, "Silent Veil", v and "Enabled" or "Disabled", 2)
    end })
    spearSection:AddToggle({ Default = VD.AutoPredict or false, Name = "Auto Predict", Flag = "Veil Auto Predict", Callback = function(v) VD.AutoPredict = v end })
    spearSection:AddSlider({ Name = "Lead Multiplier", Flag = "Veil Lead Multiplier", Min = 0.1, Max = 5, Default = VD.LeadMultiplier or 1.4, Increment = 0.1, Callback = function(v) VD.LeadMultiplier = v end })
    spearSection:AddSlider({ Name = "Max Target Distance (studs)", Flag = "Veil Max Target Distance (studs)", Min = 10, Max = 500, Default = VD.MaxTargetDistance or 500, Increment = 1, Callback = function(v) VD.MaxTargetDistance = v end })
    spearSection:AddSlider({ Name = "Spear Speed (Normal)", Flag = "Veil Spear Speed (Normal)", Min = 50, Max = 200, Default = VD.CustomSpearSpeed or 165, Increment = 1, Callback = function(v) VD.CustomSpearSpeed = v end })
    spearSection:AddSlider({ Name = "Spear Gravity (Normal)", Flag = "Veil Spear Gravity (Normal)", Min = 10, Max = 200, Default = VD.CustomSpearGravity or 103, Increment = 1, Callback = function(v) VD.CustomSpearGravity = v end })
    spearSection:AddSlider({ Name = "Aura Spear Speed", Flag = "Veil Aura Spear Speed", Min = 50, Max = 400, Default = VD.AuraSpearSpeed or 165, Increment = 1, Callback = function(v) VD.AuraSpearSpeed = v end })
    spearSection:AddSlider({ Name = "Aura Spear Gravity", Flag = "Veil Aura Spear Gravity", Min = 10, Max = 200, Default = VD.AuraSpearGravity or 96.5, Increment = 0.1, Callback = function(v) VD.AuraSpearGravity = v end })
    spearSection:AddToggle({ Default = VD.ShowTracker or false, Name = "ESP Tracker Target", Flag = "Veil ESP Tracker Target", Callback = function(v) VD.ShowTracker = v end })
    spearSection:AddToggle({ Default = VD.ShowFOV or false, Name = "Show FOV Circle", Flag = "Veil Show FOV Circle", Callback = function(v) VD.ShowFOV = v end })
    spearSection:AddSlider({ Name = "FOV Radius (pixels)", Flag = "Veil FOV Radius (pixels)", Min = 30, Max = 500, Default = VD.FovRadius or 150, Increment = 1, Callback = function(v) VD.FovRadius = v end })
...
do -- FOV Tab
    ...
    MAWWW_AddMainInfoLine(infoPanelSection, "MawwwtKiller", "Mawwwt Killer Display", "Off")
    ...
    visualSection:AddToggle({ Default = false, Name = "Mawwwt Killer Display", Flag = "Mawwwt Killer Display", Callback = function(v) 
        VD.VIS_MawwwtKiller = v 
        if v then
            StartMawwwtKiller()
        else
            StopMawwwtKiller()
...
do -- Survivor Tab
    ...
    -- [ADDED] Auto Revive toggle
    combatSurv:AddToggle({
        Default = false,
        Name = "Auto Revive (Fake Perk)",
        Flag = "Auto Revive",
        Callback = function(v)
            VD.SURV_AutoRevive = v
        end
    })
...
print("MAWWW HUB Violence District v1.5.7 Loaded")
pcall(function()
    VD_Notify("MawwwHub", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function SetupMawwwtKillerIndicator()
    ...
    getgenv().MAWWW_MawwwtKillerGui = nil
    MAWWW_SetMainInfoPanelText("MawwwtKiller", "Mawwwt Killer Display", "Waiting...")
end

function StartMawwwtKiller()
    if getgenv().MAWWW_MawwwtKillerRunning then return end
    getgenv().MAWWW_MawwwtKillerRunning = true
    SetupMawwwtKillerIndicator()
    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_MawwwtKiller and _genv.MAWWW_MawwwtKillerRunning do
            ...
            if nk then
                MAWWW_SetMainInfoPanelText("MawwwtKiller", "Mawwwt Killer Display", "Mawwwt Killer: " .. (nk == LocalPlayer and "YOU" or nk.DisplayName or nk.Name))
            else
                MAWWW_SetMainInfoPanelText("MawwwtKiller", "Mawwwt Killer Display", "Mawwwt Killer: None")
            end
            task.wait(2)
        end
    end)
...
getgenv().MAWWW_SyncLoadedFeatures = function()
    ...
    -- Sync Mawwwt Killer Display
    if VD.VIS_MawwwtKiller then
        pcall(StartMawwwtKiller)
    else
        pcall(StopMawwwtKiller)
    end
